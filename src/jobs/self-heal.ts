...
function shouldHeal(ctx: FailureContext, tmuxSession?: string): boolean {
  ...
  const now = Date.now();
  const recentCount = recentHeals.filter((h) => now - h.at < CAP_WINDOW_MS).length;
...
export async function triggerSelfHeal(ctx: FailureContext, bot: Bot): Promise<void> {
  if (!config.SELF_HEAL_ENABLED) return;

  const key = `${ctx.source}:${ctx.name}`;

  if (!shouldHeal(ctx)) {
    logger.debug("self-heal:skipped", { source: ctx.source, name: ctx.name, key });
    return;
  }
  ...
  try {
    ...
    if (memoryEnabled) {
      getSupabase()
        ...
        .then(({ error }) => {
          if (error) logger.warn("self-heal:log-error", { error: error.message });
        });
    }
...
export async function resolveHeal(jobId: string, outcome: string): Promise<void> {
  ...
  if (memoryEnabled) {
    getSupabase()
      ...
      .then(({ error }) => {
        if (error) logger.warn("self-heal:resolve-log-error", { error: error.message });
      });
  }
}
