 ...
 *     "/home/na/brain-vault/30 - Resources/session-nuggets/api-cost-reduction/"
 *
...
async function getPlaylistVideos(): Promise<Array<{ id: string; title: string }>> {
...
async function getPlaylistVideosViaYtDlp(): Promise<Array<{ id: string; title: string }>> {
...
async function fetchTranscript(
  ...
  while ((match = regex.exec(xml)) !== null) {
    const text = match[3]
      ...
      .replace(/\n/g, " ")
      .trim();
    entries.push({ start: parseFloat(match[1]), dur: parseFloat(match[2]), text });
  }
...
async function fetchTranscriptViaYtDlp(videoId: string): Promise<string | null> {
  ...
  return new Promise((resolve) => {
    const proc = spawn("yt-dlp", [
      ...
      "--output", `/tmp/yt-playlist-${videoId}.%(ext)s`,
      "--no-warnings",
      `https://www.youtube.com/watch?v=${videoId}`,
    ]);
    proc.on("close", () => {
      if (existsSync(tmpFile)) {
        const content = Bun.file(tmpFile).text().then((vtt) => {
          // Clean VTT
          let text = vtt
            .replace(/^WEBVTT[\s\S]*?\n\n/, "")
...
async function main() {
  ...
  const results: Array<{ video: { id: string; title: string }; success: boolean; chars: number }> = [];

  for (let i = 0; i < videos.length; i++) {
    const video = videos[i];
    const safeTitle = video.title.replace(/[^a-zA-Z0-9\s-]/g, "").trim().slice(0, 60);
    const outputPath = join(outputDir, `${String(i + 1).padStart(2, "0")}-${video.id}-${safeTitle}.md`);
    ...
    try {
      ...
      if (transcript.length > 0) {
        const lines = transcript
          ...
          .join("\n");
        const duration = transcript[transcript.length - 1]?.start + transcript[transcript.length - 1]?.dur || 0;
...
`;
      ...
      results.push({ video, success: !!transcript.length || !!ytDlpFallback, chars: content.length });
    } catch (err) {
